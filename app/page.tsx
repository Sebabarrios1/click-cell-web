...
export default function Home() {
  ...
  useEffect(() => {
    Papa.parse(LINK_EXCEL_CSV, {
      ...
      complete: (results: any) => {
        // Filtramos solo filas que tengan nombre de producto
        const datos = results.data.filter((p: any) => {
          const nombre = p.Producto || p.producto || p.PRODUCTO;
  ...
  const destacados = productos.filter((p: any) => {
    const valor = (p.Destacado || p.destacado || p.DESTACADO || "").toString().trim().toUpperCase();
    const stock = (p.Stock || p.stock || p.STOCK || "").toString().trim().toUpperCase();
    return valor === "SI" && stock === "SI";
  });
    ...
    <main className="min-h-screen bg-[#f1f5f9] text-slate-800 font-sans selection:bg-blue-100">
      ...
      <header className="relative py-20 md:py-32 overflow-hidden bg-white">
        <div className="max-w-6xl mx-auto px-6 relative z-10 text-center md:text-left">
          ...
          <h2 className="text-6xl md:text-8xl font-black mb-6 tracking-tighter italic leading-[0.9] text-slate-900 text-left">
...
// --- FUNCIÓN DE PRECIO CON BUSCADOR DE COLUMNAS ---
function formatPrecio(precio: number, item: any) {
  // Buscamos cualquier columna que se parezca a "Moneda"
  const keys = Object.keys(item);
  const keyMoneda = keys.find(k => k.toLowerCase().trim() === "moneda");
  const valorMoneda = keyMoneda ? item[keyMoneda].toString().toLowerCase().trim() : "";
...
function CardProducto({ producto, esDestacado, onClick }: any) {
  const fotosCol = Object.keys(producto).find(k => k.toLowerCase().includes("url")) || "ImagenURL";
  const fotos = (producto[fotosCol] || "").split(",").map((s: string) => s.trim()) || [];

  const nombreCol = Object.keys(producto).find(k => k.toLowerCase() === "producto") || "Producto";
  const nombre = producto[nombreCol];

  const precioCol = Object.keys(producto).find(k => k.toLowerCase().includes("base")) || "PrecioBase";
  const precioLimpio = producto[precioCol] || "0";
  const precioBase = parseFloat(precioLimpio.toString().replace(/\$/g, "").replace(/\./g, "").trim()) || 0;

  const descCol = Object.keys(producto).find(k => k.toLowerCase().includes("descuento")) || "Descuento";
  const desc = parseFloat(producto[descCol]) || 0;
  const precioFinal = precioBase - (precioBase * (desc / 100));

  return (
    <div onClick={onClick} className="group cursor-pointer relative bg-white border border-slate-100 rounded-[2rem] overflow-hidden hover:shadow-xl transition-all duration-500 flex flex-col h-full shadow-md">
      ...
      <div className="p-8 flex flex-col flex-grow text-left">
        <h3 className="font-black tracking-tighter text-slate-900 mb-4 uppercase leading-none text-xl">{nombre}</h3>
...
function ModalProducto({ producto, onClose }: any) {
  ...
  const fotosCol = keys.find(k => k.toLowerCase().includes("url")) || "ImagenURL";
  const fotos = (producto[fotosCol] || "").split(",").map((s: string) => s.trim()) || [];
  const [index, setIndex] = useState(0);

  const nombreCol = keys.find(k => k.toLowerCase() === "producto") || "Producto";
  const precioCol = keys.find(k => k.toLowerCase().includes("base")) || "PrecioBase";
  const precioLimpio = producto[precioCol] || "0";
  const precioBase = parseFloat(precioLimpio.toString().replace(/\$/g, "").replace(/\./g, "").trim()) || 0;

  const descCol = keys.find(k => k.toLowerCase().includes("descuento")) || "Descuento";
  const desc = parseFloat(producto[descCol]) || 0;
  const precioFinal = precioBase - (precioBase * (desc / 100));

  const descripCol = keys.find(k => k.toLowerCase().includes("descrip")) || "Descripcion";
  const descrip = producto[descripCol] || "";
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 md:p-10 bg-slate-900/60 backdrop-blur-md overflow-y-auto">
      <div className="bg-white w-full max-w-5xl rounded-[3rem] overflow-hidden shadow-2xl flex flex-col md:flex-row relative">
        ...
        <div className="md:w-1/2 p-10 md:p-16 flex flex-col justify-center text-left">
          <h2 className="text-4xl md:text-5xl font-black mb-6 tracking-tighter uppercase text-slate-900 italic leading-none">{producto[nombreCol]}</h2>
          ...
          <a href={`https://wa.me/${WHATSAPP_NUMBER}?text=Hola Click Cell! Info del ${producto[nombreCol]}`} target="_blank" className="block w-full bg-slate-900 text-white text-center py-5 rounded-2xl font-black uppercase tracking-[0.2em] hover:bg-blue-600 transition-all shadow-xl shadow-slate-900/10">CONSULTAR AHORA</a>
        </div>
      </div>
    </div>
  );
